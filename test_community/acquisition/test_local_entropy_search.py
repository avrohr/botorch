...
class TestLocalEntropySearch(BotorchTestCase):
    ...
    def test_default_chunking_matches_explicit_full_batching(self) -> None:
        ...
        X = torch.rand(6, 1, 3, device=self.device, dtype=dtype)

        torch.manual_seed(2024)
        acqf_default = LocalEntropySearch(
            model=model,
            x_incumbent=train_X[0],
            num_path_samples=4,
            num_descent_steps=4,
        ...
        self.assertAllClose(acqf_default(X), acqf_full(X), atol=1e-7, rtol=1e-5)
    ...
    def test_to_updates_cached_conditional_models(self) -> None:
        train_X, model = self._get_model(dtype=torch.double)
        acqf = LocalEntropySearch(
            model=model,
            x_incumbent=train_X[0],
            num_path_samples=3,
        ...
        X = torch.rand(4, 1, 2, device=self.device, dtype=torch.float)
        values = acqf(X)
        self.assertEqual(values.dtype, torch.float)
        self.assertTrue(torch.isfinite(values).all())

    def test_optimizer_step_must_not_require_closure(self) -> None:
        train_X, model = self._get_model()

        with self.assertRaisesRegex(
            ValueError, "must not require additional arguments"
        ):
            LocalEntropySearch(
                model=model,
                x_incumbent=train_X[0],
